...
claim. Most entries do not yet record whether the finder was a person or an AI
model run against the artefact, and until they do the ratio is not evidence of
human peer review. And a programme can always raise its own ratio by looking
...
AI language models, and one of the corrections recorded in the log
(\S\ref{sec:selfdefect}) was found by an AI model run against the artefact
rather than by a human reviewer. Every number reported here was recomputed by
the author; all registered predictions, thresholds, verdicts and corrections
are the author's own and are the author's responsibility. The correction log
records, per entry, whether the finder was the author, a person, or an AI
model, so that the external-finder ratio in \S\ref{sec:discussion} can be read
for what it is.
