...
The two figures in these paragraphs are quoted from that audit's corrigendum
and are \emph{not} anchored under \S\ref{sec:threshold}: that audit predates the
anchoring requirement and its reissue is outstanding. They are used here to
illustrate the two requirements, not as results of the present work.
...
\paragraph{The audited object is not independently citable.} The SRAG/SRLG
documents are unpublished manuscripts held by the author; the pre-registration
identifies them by internal filename and page. A reader therefore cannot verify
that the corpus states the quantities P2 tests --- $C(0.08) = 0.18$, the
$0.24$\,rad headline, the $2.7\times10^{-22}$\,rad/Mpc rate. P2's verdicts are
consequently reproducible from the formulas but not falsifiable against the
source, and we do not ask them to carry weight beyond that. P1 and P3--P9 do
not depend on the corpus text: they test properties of the stated functional
form, which is reproduced in full above and in
\texttt{audits/gw-srag/code/srag\_core.py}.
...
The table as shipped in v0 reported $\max|\rho| = 0.952$ and a profile
...
unchanged; every value in the row was wrong, and the v0 figure was
...
Across three audits this programme has logged twenty corrections to audit
claims, eighteen of them found by someone other than the author, together with
eight corrections to the release itself. The counting rule and the per-entry
finder provenance are published alongside, and the totals are derived from the
log rather than maintained beside it, because an earlier version of this
sentence quoted three mutually inconsistent numbers.

We propose that this ratio, rather than the absence of corrections, is the
appropriate quality signal for an audit programme: an author whose published
error rate is zero is an author who is not being checked. Two caveats bound the
claim, and the first is severe. \textbf{Every external finder in this log was
an AI language model run adversarially against the artefact. None was a human
reviewer.} This work has not been through human peer review of any kind, and
the ratio above should be read as a measure of automated adversarial scrutiny,
not of community scrutiny. We report it because we think automated adversarial
review is worth something and worth counting -- three of the findings it
produced changed the protocol itself -- but it is not a substitute, and a
reader who took the ratio for peer review would be misled. And a programme can
always raise its own ratio by looking
harder at itself, so the metric is a floor on scrutiny received, not a ceiling
on quality.
...
\url{https://doi.org/10.5281/zenodo.22649089}. The gravitational-wave audit is a forecast against published
constraints; no strain data were used. Reproduce with
\texttt{python audits/gw-srag/code/reproduce.py -{}-all}.

\section*{Use of AI tools}

Drafting of prose and implementation of software in this work were assisted by
AI language models. So was the review: of the twenty-eight corrections in the
log, twenty-six were found by AI models run adversarially against the artefact
and two by the author. \textbf{No part of this work has been reviewed by a
human other than its author.} Among the AI-found corrections are the one that
motivated the protocol change in \S\ref{sec:threshold}, the detection limits
in \S\ref{sec:gw}, and the presence of superseded values inside a shipped
result file.

Every number reported here was recomputed by the author; all registered
predictions, thresholds, verdicts and corrections are the author's own and are
the author's responsibility. The correction log records the finder kind per
entry so the ratio in \S\ref{sec:discussion} can be checked rather than taken
on trust.
