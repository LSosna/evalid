...
"""Reproducer for EVALID-AUDIT-GW-SRAG. Closes release-hygiene findings R1-2/3/4.
    ...
    python reproduce.py --ff          # P5 fitting factors (slow, ~7 min)
    python reproduce.py --all         # all five result files, rewrites results/

--all regenerates every file in results/ except the two figures. It is the
only supported way to rebuild them; editing a results file by hand will be
caught by `evalid manifest verify`.
...
def ff_blocks() -> tuple[dict, dict, dict]:
    """Regenerate t3_ff, t4_rest and t5_derived in their shipped schema.

    Before v0.3.4 these three were archival: no code in the repository produced
    them, `--all` silently wrote a different schema over t3_ff.json, and
    t4_rest.json still carried the retracted v0 P6 table. All three are now
    generated here, from ff_engine, with the ladder solved by warm starts.
    """
    from gwfisher import make_grid, fisher
    from ff_engine import ladder, fitting_factor, significance, A_for_significance, SEED

    f, S = make_grid(MC, ETA, 20.0, None, 20000)
    env_ = dict(env(), grid_points=20000, n_restarts=5)

    deg = {}
    for name, mc, eta in (("BNS_GW170817like", MC, ETA),
                          ("BBH_GW150914like", 30.0, ETA)):
        ff_, SS = make_grid(mc, eta, 20.0, None, 20000)
        F, cov, sig, corr, _ = fisher(mc, eta, CHI, ff_, SS, snr=25.0)
        cond = float(np.linalg.cond(F))
        e = {"cond_number": cond, "registered_ceiling": COND_CEILING,
             "gate": "OK" if cond <= COND_CEILING else "UNDERDETERMINED"}
        if cond <= COND_CEILING:
            e.update({"sigma_marg": float(sig[-1]),
                      "sigma_cond": float(1 / np.sqrt(F[-1, -1])),
                      "degradation": float(sig[-1] * np.sqrt(F[-1, -1]))})
        else:
            e["note"] = ("quantities withheld: protocol 4.3 gate failed, "
                         "verdict UNDERDETERMINED")
        deg[name] = e

    A_LADDER = [0.0, 0.01, 0.05, 0.1, 0.3, 0.705, 0.761, 2.743, 5.0, 8.889, 14.529]
    scan = ladder(A_LADDER, f, S)
    A3_25, fit25 = A_for_significance(scan, 3.0, 25.0)
    A3_100, fit100 = A_for_significance(scan, 3.0, 100.0)
    A_floor = 0.7052304682876398

    keep = ("A_log", "dPhi_50_200_rad", "FF", "sig_snr25", "sig_snr100",
            "restart_spread_FF")
    t3 = {"note": ("GR-only template maximised against a log-dispersed BNS "
                   "injection; mc=1.188 Msun, eta=0.2497, chi=0, 20-1610 Hz, "
                   "aLIGO-shape PSD. eta bounded <0.25 and |chi|<=1 in the "
                   "optimiser. Regenerated by code/reproduce.py --ff."),
          "degeneracy": deg,
          "ff_scan_BNS": [{k: r[k] for k in keep} for r in scan],
          "A_3sigma_snr25": A3_25, "A_3sigma_snr100": A3_100,
          "A_floor_over_all_lambda": A_floor,
          "detection_limit_fit": {"snr25": fit25, "snr100": fit100},
          "environment": env_}

    LAD = {0.761, 2.743, 5.0, 8.889, 14.529}
    t4 = {"ff_ladder": [{"A_log": r["A_log"], "dPhi": r["dPhi_50_200_rad"],
                         "FF": r["FF"], "mc_frac_bias": r["mc_frac_bias"],
                         "eta_rec": r["eta_rec"], "chi_rec": r["chi_rec"],
                         "sig25": r["sig_snr25"], "sig100": r["sig_snr100"],
                         "restart_spread_FF": r["restart_spread_FF"]}
                        for r in scan if r["A_log"] in LAD],
          "A3sigma_snr25": A3_25, "A3sigma_snr100": A3_100,
          "separability": {"_moved": "P6 lives only in p6_corrected.json. The v0 "
                                     "values previously duplicated here are "
                                     "RETRACTED - see CORRIGENDUM_P6.md.",
                           "source": "results/p6_corrected.json"},
          "stats": {"P7_p_r0p5_n15": None, "P7_r_required": None,
                    "P7_n_required": None, "P8_fpath_3kpc_40Mpc": None,
                    "P8_fpath_20kpc_40Mpc": None, "P9_n_EM_counterparts": 1},
          "environment": env_}
    st = internal()
    t4["stats"].update({"P7_p_r0p5_n15": st["P7"]["p_at_r0.5_n15"],
                        "P7_r_required": st["P7"]["r_needed_p0.01_n15"],
                        "P7_n_required": st["P7"]["n_needed_p0.01_r0.5"],
                        "P8_fpath_3kpc_40Mpc": st["P8_host_path_fraction"]["3kpc_at_40Mpc"],
                        "P8_fpath_20kpc_40Mpc": st["P8_host_path_fraction"]["20kpc_at_40Mpc"]})

    r008 = next(r for r in scan if r["A_log"] == 0.761)
    rsp = next(r for r in scan if r["A_log"] == 14.529)
    s008 = r008["sig_snr25"]
    t5 = {"SNR_needed_3sigma_lam0.08": 25.0 * 3.0 / s008,
          "SNR_needed_3sigma_A_floor": 25.0 * 3.0 / significance(
              fitting_factor(A_floor, f, S, n_restarts=3)["FF"], 25.0),
          "N_events_3sigma_lam0.08_snr25": (3.0 / s008) ** 2,
          "sig_snr25_SPARC_lam": rsp["sig_snr25"],
          "GW170817_network_SNR": 32.4,
          "note": ("Ladder points 0.761 and 14.529 are A(lambda) rounded to "
                   "three decimals for lambda = 0.08 and 2.4e-8. Significances "
                   "are quoted at those ladder points, so they differ in the "
                   "third decimal from values computed at unrounded A."),
          "environment": env_}
    return t3, t4, t5
...
def main(argv=None) -> int:
    ...
    if args.ff or args.all:
        t3, t4, t5 = ff_blocks()
        _dump("t3_ff.json", t3)
        _dump("t4_rest.json", t4)
        _dump("t5_derived.json", t5)
